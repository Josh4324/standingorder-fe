...
const StandingOrderForm = () => {
      ...
      fetch(`${api_base_url}/api/verify/internal`, {
        ...
        .then((data) => {
          ...
          if (JSON.stringify(data.accountName) === "{}") {
            ...
            if (
              Number(data.accountName.AccountProductCode) < 4 ||
              Number(data.accountName.AccountProductCode) > 99
            ) {
              NotificationManager.error(
                "This account is not eligible to create a standing order",
                "Error"
              );
              setTimeout(() => {
                history.push("/");
  ...
  const submit = (evt) => {
    ...
    datalist.map((item) => {
      let narration =
        "From " + accountname + " | " + "Remarks - " + item.remark;
          ...
          <CCol xs="10" style={{ marginLeft: "auto", marginRight: "auto" }}>
            ...
            <CCard>
              ...
              <CCardBody>
                ...
                <CInputGroup
                  className={error.length > 0 ? "alert alert-danger" : "none"}
                >
                  <div>{error}</div>
                </CInputGroup>
              </CCardBody>
            ...
            <CCardFooter className="mb-3">
              ...
              <CButton type="submit" size="md" className="mr-3" color="primary">
                Submit
              </CButton>
              <CButton type="reset" onClick={reset} size="md" color="danger">
                Reset
              </CButton>
            </CCardFooter>
          </CCol>
