  ...
  const token = localStorage.getItem('stand-order-token')
    ...
    .then(data => {
        if (data.status === "Success"){
            NotificationManager.success("Stand order cancelled, awaiting deactivation approval", "Success");
            setTimeout(() => {
              window.location.reload(false);
            }, 3000)
        }else{
            NotificationManager.error("Error trying to cancel Stand order", "Error");
        }
    })
    .catch((error) => {
